...
def makeRequest(url, headers=None):
    try:
        if headers is None:
            headers = {'User-Agent': '(Linux;Android 7.0) ExoPlayerLib/2.5.3'}
        ...
        for other,channel_name,stream_url in match2:
            ...
            if cat:
                ...
                if not 'plugin' in stream_url and not 'User-Agent' in stream_url and not 'Referer' in stream_url and not 'Origin' in stream_url and not 'Cookie' in stream_url:
                    stream_url = stream_url + '|User-Agent=(Linux;Android 7.0) ExoPlayerLib/2.5.3'
                if '.m3u8' in stream_url and f4mtester and not 'pluto.tv' in stream_url and not 'plugin' in stream_url:
                    plugin_m3u8 = 'plugin://plugin.video.f4mTester/?url={0}&streamtype=HLSRETRY&iconImage={1}&name={2}[COLOR yellow] GIF [COLOR gold]PEG [COLOR red]TV [/COLOR]'.format(urllib_parse.quote_plus(stream_url),urllib_parse.quote_plus(thumbnail),urllib_parse.quote_plus(channel_name))
                    ...
                    plugin_ts = 'plugin://plugin.video.f4mTester/?url={0}&streamtype=TSDOWNLOADER&iconImage={1}&name={2}[COLOR yellow] GIF [COLOR gold]PEG [COLOR red]TV [/COLOR]'.format(urllib_parse.quote_plus(stream_url),urllib_parse.quote_plus(thumbnail),urllib_parse.quote_plus(channel_name))
